...
void load_goal_into_buffer(GameState *game_state, BufferAndCount *buffer){
    ...
    };
    {
        ...
        { // We draw the flag
            ...
            {
                const RgbaColor pcolor = {120, 120, 120, 95};
                const float z = 0.1f;
            ...
            if(level_completed){
                ...
                level->flag_y += 15.f*TIME_STEP;
                if(level->flag_y > level->flag_top_y)
                    level->flag_y = level->flag_top_y;
            }
            
            Vec3 oldr0 = r, oldr1 = {r.x, r.y+flag_height, r.z};
            float bonus_slope = min(min(level->flag_top_y-level->flag_y, 0.66f), level->flag_y-level->flag_base_y-0.5f) * (0.2f / flag_segments_plus_1);
            for(int i=0; i<flag_segments; i++){
                r.x += flag_segment_length;
                r.y += 0.5f * flag_height / flag_segments_plus_1 + bonus_slope;
            ...
            r.x += flag_segment_length;
            r.y += 0.5f*flag_height/flag_segments_plus_1 + 0.05f*cos(8.f*r.x+2.f*t) + bonus_slope;
            *(vertices++) = {oldr0, color};
            *(vertices++) = {oldr1, color};
            *(vertices++) = {r, color};
        }
